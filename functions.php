...
wp_enqueue_script( 'infinitescroll', get_template_directory_uri() . '/javascripts/jquery.infinitescroll.min.js', array('jquery'), '', false );
...
wp_enqueue_script( 'app', get_template_directory_uri() . '/javascripts/app.js?' . filemtime( get_stylesheet_directory() . '/style.css'), array('foundation', 'jquery', 'infinitescroll'), '', true );
...
wp_register_style('aude-layout', get_template_directory_uri() . '/style.css?' . filemtime( get_stylesheet_directory() . '/style.css'));
...
'before_title'  => '<h4 class="border-top-normal">',
'after_title'   => "</h4>"
));
...
function aude_comment($comment, $args, $depth) {
    ...
    <div class="comment-time"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php comment_date(get_option('date_format')); ?> - <?php comment_time('H:i:s'); ?></a></div>
